...
from .api import *
from .packets import OesRspMsgBodyT


# wrapper
class OesApi:
    def __init__(self):
        lib_file_path = str(Path(__file__).resolve().parent.parent.joinpath('liboes_api.so'))
        self.api = cdll.LoadLibrary(lib_file_path)
    ...
    def OesApi_QuerySingleCashAsset(self, cash_acct_id, out_cash_asset_item):
        ...
        return self._OesApi_QuerySingleCashAsset(byref(self.qry_channel),
                                                 None if cash_acct_id is None else create_string_buffer(
                                                     cash_acct_id.encode()),
    ...
    def OesApi_QuerySingleStkHolding(self, inv_acct_id, security_id, out_holding_item):
        ...
        return self._OesApi_QuerySingleStkHolding(byref(self.qry_channel),
                                                  None if inv_acct_id is None else create_string_buffer(
                                                      inv_acct_id.encode()),
                                                  None if security_id is None else create_string_buffer(
                                                      security_id.encode()),
    ...
    def OesApi_SetCustomizedDriverId(self, driver_id):
        ...
        return self._OesApi_SetCustomizedDriverId(
            None if driver_id is None else create_string_buffer(driver_id.encode()))
